...
以财务系统 ea_dy_account_classify 为主，按名称关联进销存 ba_account_classify：
- 名称能匹配上：回写 ba_account_classify.finance_classify_id
...
class AccountClassifyLinkWorker(QThread):
    """账户分类关联工作线程"""
    ...
    def run(self):
        ...
        try:
            ...
            source_cursor.execute("""
                SELECT id, name, remark, type
                FROM ea_dy_account_classify
            ...
            self.log_message.emit(f"财务系统共 {total} 条账户分类")
            ...
            # 进销存现有账户分类（按名称索引，可能重名）
            target_cursor.execute("""
                SELECT id, name, finance_classify_id
                FROM ba_account_classify
                WHERE (delete_time IS NULL OR delete_time = 0)
                ORDER BY id ASC
            """)
            ...
            ba_by_name = {}
            for row in ba_rows:
                ba_by_name.setdefault((row['name'] or '').strip(), []).append(row)
            self.log_message.emit(f"进销存现有 {len(ba_rows)} 条账户分类")
            ...
            for index, ea in enumerate(ea_rows):
                ...
                try:
                    ...
                    if candidates:
                        ...
                        result_row['进销存分类ID'] = ba_row['id']
                        result_row['原关联财务ID'] = ba_row['finance_classify_id'] or ''

                        if ba_row['finance_classify_id'] == ea['id']:
                            self.results['skip_count'] += 1
                        ...
                        if not self.preview_only:
                            target_cursor.execute("""
                                UPDATE ba_account_classify
                                SET finance_classify_id = %s, update_time = %s
                                WHERE id = %s
                            """, (ea['id'], now, ba_row['id']))
                            target_conn.commit()

                        self.results['bind_count'] += 1
                        if ba_row['finance_classify_id']:
                            result_row['处理状态'] = ('可改绑' if self.preview_only else '已改绑') + \
                                f"(原财务ID={ba_row['finance_classify_id']})"
                        else:
                            result_row['处理状态'] = '可关联' if self.preview_only else '已关联'
                        self.results['completed_data'].append(result_row)
                        continue
                    ...
                    if not self.preview_only:
                        target_cursor.execute("""
                            INSERT INTO ba_account_classify
                                (name, remark, type, finance_classify_id, create_time, update_time)
                            VALUES (%s, %s, %s, %s, %s, %s)
                        """, (name, ea['remark'], ea['type'] if ea['type'] is not None else 0,
                              ea['id'], now, now))
                        target_conn.commit()
                        new_id = target_cursor.lastrowid
                        used_ba_ids.add(new_id)
                        result_row['进销存分类ID'] = new_id
            ...
            for row in ba_rows:
                if (row['name'] or '').strip() not in ea_names:
                    self.log_message.emit(f"提示: 进销存分类「{row['name']}」财务系统没有，未处理")
            ...
            self.log_message.emit(f"财务分类总数: {total}")
...
class AccountClassifyLinkWidget(QWidget):
    """账户分类关联财务系统界面"""
...
<b>处理逻辑:</b><br>
• 遍历财务库 ea_dy_account_classify 中未删除的分类<br>
• 按 name 匹配进销存 ba_account_classify.name<br>
• 匹配上：回写 ba_account_classify.finance_classify_id（已关联到其他ID会改绑并记录）<br>
• 匹配不上：在进销存新增一条(name/remark/type)并直接绑定<br>
• 进销存有、财务没有的分类：仅在日志提示，不处理<br><br>
<b>注意:</b> 银行名称关联依赖本功能的结果，请先跑完本功能再跑「银行名称关联财务系统」！<br>
<b>建议先执行「预检查」确认结果后再执行「开始同步」</b>
    ...
    def start_sync(self):
        ...
        reply = QMessageBox.question(
            ...
            f"将回写 ba_account_classify.finance_classify_id，并新增财务有、进销存没有的分类\n"
            f"操作不可撤销，请确认！",
    ...
    def _run_worker(self, target_ds, source_ds, preview_only):
        ...
        self.worker = AccountClassifyLinkWorker(target_ds, source_ds, preview_only=preview_only)
    ...
    def on_finished(self, results):
        ...
        text = (f"财务分类{results['total']}条 | 关联: {results['bind_count']} | "
    ...
    def export_completed(self):
        if not self.completed_data:
            ...
            return
        default_name = f"账户分类关联完整记录_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    ...
    def export_failed(self):
        if not self.failed_records:
            ...
            return
        default_name = f"账户分类关联失败记录_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
